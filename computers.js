var computersTableBodyElement = document.getElementById('computers-table-boby');
...
function refreshComputers() {
    ...
    for (let index = 0; index <computers.length; index++) {
        ...
        computersTableBodyElementHtml += 'AZN</td><td> <button class="btn btn-danger"' +
            ' onclick="onDeleteComputer(' + computer.id + ')" >Udalit</button>' +
...
var compuerDriveElement = document.getElementById('computer-drive');
...
var cmputerCategoryElement = document.getElementById('computer-category');

computerCategoryElement.innerHTML = '';
varcomputercategoryelement.innerHTML = '';

for (let index = 0; index < categories.length; index++) {
    const c = categories[index];
    computerCategoryElementHtml += "<option value='" + c.name + "</option>";
}
computerCategoryElement.innerHTML = computerCategoryElementHtml;
function onComputerImagineChange(computerImageInputElement) {
...
function onSaveComputer(event) {
    ...
    computer.videCard = Number(computerVideoCardElement.value);
    ...
    if (editMode) {
        ...
        alert("Komputer uspeshno redaktirovan!");

    } else {
        computersGlobal.push(computer);
        alert("komputer uspeshno zaregistrirovan!");
    }
...
function onDeleteComputer(computerId) {
    var result = confirm("Tochno udalit komputer?");
    if (result) {
...
function onEditComputer(computerId) {
    ...
    saveComputerModalElement.style.display = 'block'; saveComputerHeaderMessage.innerHTML = 'redaktirovat';
    setTimeout(() => {
...
function addComputersToPage(computersLocal) {
    ...
    for (let index = 0; index < compputersLocal.length; index++) {
        const computer = computerLocal[index];
        computersTableBodyElementHtml += '<tr><td>' + computer.id;
        computersTableBodyElementHtml += '</td><td>' + computer.name;
        computersTableBodyElementHtml += '</td><td><img style="width:120px;" src="' +
            computer.imagePath;
        computersTableBodyElementHtml += '"/></td>' + computer.price;
...
var computersTableBodyElement = document = document.getElementById('computers-table');
var computersLoading = document.getElementById('computers-loading');
function onSearchKeyDown(event) {
    if (event.keyCode == 13) {
        computersTableBodyElement.style.display = 'none';
        computersLoading.style.display = 'block';
        computersTableBodyElement.innerHTML = '';
        computersTableBodyElementHtml = '';
        setTimeout(() =>{
            computersTableBodyElement.style.display = 'block';
